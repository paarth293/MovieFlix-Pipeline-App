...
try:
    ...
    from sklearn.neighbors import KNeighborsClassifier, DBSCAN
        ...
        if st.button("🔧 Apply Imputation", key="apply_impute"):
            ...
            for col in num_cols:
                if df_imp[col].isnull().sum() > 0:
                    if num_strategy == "Mean": df_imp[col].fillna(df_imp[col].mean(), inplace=True)
                    elif num_strategy == "Median": df_imp[col].fillna(df_imp[col].median(), inplace=True)
                    elif num_strategy == "Mode": df_imp[col].fillna(df_imp[col].mode()[0], inplace=True)
                    elif num_strategy == "Zero": df_imp[col].fillna(0, inplace=True)
            for col in cat_cols:
                if df_imp[col].isnull().sum() > 0:
                    if cat_strategy == "Mode": df_imp[col].fillna(df_imp[col].mode()[0], inplace=True)
                    else: df_imp[col].fillna("Missing", inplace=True)
    ...
    with method_tab3:
        if not SKLEARN_OK:
            ...
            try:
                ...
                mi_scores = mi_func(X[num_features], y, random_state=42)
...
def pipeline_step_5():
    ...
    with col2:
        stratify = st.checkbox("Stratified split", value=(st.session_state.problem_type == "Classification"), key="stratify")
    with col3:
    ...
    if st.button("✂️ Split Data", key="split_btn", use_container_width=True):
        try:
            strat = y_encoded if (stratify and st.session_state.problem_type == "Classification") else None
            X_tr, X_te, y_tr, y_te = train_test_split(
                X_all, y_encoded, test_size=test_size,
                stratify=strat, random_state=int(random_state)
            )
    ...
    if st.button("🏋️ Train & Cross-Validate", key="train_btn", use_container_width=True):
        with st.spinner(f"Training {st.session_state.model_name} with {k}-fold CV…"):
            try:
                ...
                cv = (StratifiedKFold(n_splits=k, shuffle=shuffle, random_state=42) if (is_clf and shuffle)
                      else KFold(n_splits=k, shuffle=shuffle, random_state=42))
